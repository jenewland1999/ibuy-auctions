...
<?php else: ?>
  <div class="col-12">
    <div class="auction my-3">
      <!-- Card Image Carousel -->
      <section id="carousel<?= $auction['auction_id'] ?>" class="auction__images carousel slide carousel-fade" date-ride="carousel">
        <ol class="carousel-indicators">
          <?php for($i = 0; $i < 5; $i++): ?>
            <li data-target="#carousel<?= $auction['auction_id'] ?>" data-slide-to="<?= $i ?>" class="<?= $i === 0 ? 'active' : '' ?>"></li>
          <?php endfor; ?>
      ...
      <h5 class="auction__name"><?= htmlspecialchars($auction['auction_name'], ENT_QUOTES, 'UTF-8'); ?></h5>
      <h6 class="auction__category mb-2 text-muted"><?= htmlspecialchars(getCategory($pdo, $auction['category_id'])['category_name'], ENT_QUOTES, 'UTF-8'); ?></h6>
      <p class="auction__description" style="white-space: pre-wrap;"><?= htmlspecialchars($auction['auction_description'], ENT_QUOTES, 'UTF-8'); ?></p>
      <div class="auction__more text-right">
        <p class="auction__current-bid font-weight-bold mb-0" style="color:red;font-size:2rem;">Current Bid: <?= htmlspecialchars(formatCurrency(getBidCurrentPrice($pdo, $auction['auction_id']), '£'), ENT_QUOTES, 'UTF-8'); ?></p>  
        <a href="/auctions/auction.php?id=<?= $auction['auction_id']; ?>" class="btn btn-lg btn-primary mb-1 rounded-0">More &gt;&gt;</a>
      </div>
    </div>
  </div>
<?php endif; ?>
